...
public class AccountMapperImpl implements AccountMapper {
    ...
    @Override
    public Account fromSignUpStudentToAccount(SignUpStudentForm signUpStudentForm) {
        if ( signUpStudentForm == null ) {
            return null;
        }

        Account account = new Account();

        account.setAvatarPath( signUpStudentForm.getAvatarPath() );
        account.setFullName( signUpStudentForm.getFullName() );
        account.setPhone( signUpStudentForm.getPhone() );
        account.setEmail( signUpStudentForm.getEmail() );

        return account;
    }

    @Override
    public void fromUpdateStudentFormToEntity(UpdateStudentForm updateStudentForm, Account account) {
        if ( updateStudentForm == null ) {
            return;
        }

        if ( updateStudentForm.getAvatarPath() != null ) {
            account.setAvatarPath( updateStudentForm.getAvatarPath() );
        }
        if ( updateStudentForm.getFullName() != null ) {
            account.setFullName( updateStudentForm.getFullName() );
        }
        if ( updateStudentForm.getPhone() != null ) {
            account.setPhone( updateStudentForm.getPhone() );
        }
        if ( updateStudentForm.getEmail() != null ) {
